...
In this work we will focus our findings on two fiducial parameter sets, one for the standard MRI (hereafter SMRI) where $\xi = 0$ and one for the helical MRI (hereafter HMRI). Our fiducial parameters are described in Table \ref{table:parameters}.

\begin{table}[ht]\label{table:parameters}
\normalsize
\caption{Fiducial parameters for MRI runs} 
\centering
%\resizebox{\textwidth}{!}
{\begin{tabular}{rlrrrrrrr}
  \hline
 & $\xi$ & $Pm$ & $\Omega_2/\Omega_1$ & $\beta$ & X \\ 
  \hline\hline
Standard MRI & 0 & 25 & 0 & 0 & 0 \\ 
Helical MRI & 4 & 0 & 0\\ 
   \hline
\end{tabular}}
\end{table} 
