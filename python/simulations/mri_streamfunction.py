...
#streamfunction
mri.add_equation("dt(dx(psi_x)) + dz(dz(dt(psi))) - 2*dz(u) + (dx(psi_xxx) + dz(dz(dz(dz(psi)))))/Re - 2*B0/beta*(dz(A_xx) + dz(dz(dz(A)))) = 2/beta*((dx(A_xx) + dz(dz(A_x))) * dz(A) - (dz(A_xx) + dz(dz(dz(A))))*A_x) - ((psi_xxx + dz(dz(A_x))) * dz(psi) - (dz(psi_xx) + dz(dz(dz(psi))))*psi_x)")
...
mri.add_right_bc("psi = 10",condition="dz == 0")
mri.add_left_bc("psi_x = 0")
mri.add_right_bc("psi_x = 0")
